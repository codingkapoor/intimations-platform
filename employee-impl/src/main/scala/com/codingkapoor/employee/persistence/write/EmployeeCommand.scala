...
case class CreateIntimation(empId: String, intimationReq: IntimationReq) extends EmployeeCommand[Done]

object CreateIntimation {
  implicit val format: Format[CreateIntimation] = Json.format[CreateIntimation]
}

case class UpdateIntimation(empId: String, intimationReq: IntimationReq) extends EmployeeCommand[Done]

object UpdateIntimation {
  implicit val format: Format[UpdateIntimation] = Json.format[UpdateIntimation]
}

case class CancelIntimation(empId: String) extends EmployeeCommand[Done]

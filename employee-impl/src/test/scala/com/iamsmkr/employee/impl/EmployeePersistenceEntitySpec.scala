...
import com.iamsmkr.employee.api.models.PrivilegedIntimationType.{Maternity, Paternity, Sabbatical}
import com.iamsmkr.employee.api.models._
import com.iamsmkr.employee.impl.persistence.write.EmployeePersistenceEntity.{already5, balanceExtraWithNewCredits, computeCredits, getNewLeaves, isWeekend}
import com.iamsmkr.employee.impl.persistence.write.{EmployeePersistenceEntity, EmployeeSerializerRegistry}
...
class EmployeePersistenceEntitySpec extends WordSpec with Matchers with BeforeAndAfterAll with OptionValues with TypeCheckedTripleEquals {
  ...
  "Employee persistence entity" should {
    ...
    "ignore update of an already existing employee when employeeInfo has no changes" in withDriver { driver =>
      ...
      outcome.replies should be(List(employee))
      outcome.issues should be(Nil)
    }
    ...
    "credit leaves when there is an inactive intimation and an inactive maternity privileged intimation" in withDriver { driver =>
      val today = LocalDate.now()
      val yesterday = today.minusDays(1)
      val endDate = if (isWeekend(yesterday)) yesterday.minusDays(2) else yesterday
      val startDate = if (isWeekend(yesterday.minusDays(4))) yesterday.minusDays(6) else yesterday.minusDays(4)
    ...
    "credit leaves when there is an inactive intimation and an inactive paternity privileged intimation" in withDriver { driver =>
      val today = LocalDate.now()
      val yesterday = today.minusDays(1)
      val endDate = if (isWeekend(yesterday)) yesterday.minusDays(2) else yesterday
      val startDate = if (isWeekend(yesterday.minusDays(4))) yesterday.minusDays(6) else yesterday.minusDays(4)
    ...
    "credit leaves when there is an active intimation and a non-existent privileged intimation" in withDriver { driver =>
      val today = LocalDate.now()
      val tomorrow = today.plusDays(1)
      val requestDate = if (isWeekend(tomorrow)) tomorrow.plusDays(2) else tomorrow
      val requests = Set(Request(requestDate, RequestType.Leave, RequestType.Leave))
      ...
      val is@initialState = state.copy(activeIntimationOpt = Some(activeIntimation))
      driver.initialize(Some(Some(initialState)))
